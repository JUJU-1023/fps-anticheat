...
#
#  처리 타입: match_start / match_end / move / violation / combat / ban_enforced
#
#  ban_enforced (W9 Day 5)
#   게임 서버 BanEnforcer 가 킥/밴을 집행할 때 남긴다.
#   bans.enforced_at 을 "처음 집행한 시각"으로 채운다 (COALESCE 로 덮어쓰지 않음).
#   재접속 거부가 반복돼도 첫 시각이 유지되고, 각 시도는 서버 로그에 남는다.
#   gs 계정의 UPDATE 권한으로 충분하다.
#
#  실행
...
# 처음 집행 시각만 남긴다. 재접속 거부가 반복돼도 덮어쓰지 않는다.
ENFORCED_SQL = """
UPDATE bans SET enforced_at = COALESCE(enforced_at, %s) WHERE id = %s
"""
    ...
    with db.conn.cursor() as cur:
        for r in records:
            ...
            if t == "match_start":
                ...
                # players/matches 행을 만들 필요가 없다. bans.id 로 바로 갱신.
                bid = r.get("ban_id")
                if bid is not None:
                    enforced.append((parse_ts(r.get("ts")), bid))
        ...
        if enforced:
            cur.executemany(ENFORCED_SQL, enforced)

    db.conn.commit()
    return len(moves), len(viols), len(combats), len(enforced)
...
def process_file(db, path, name, ckpt, batch_size):
    ...
    total_m = total_v = total_c = total_e = 0
    try:
        for i in range(0, len(records), batch_size):
            m, v, c, e = apply_batch(db, records[i:i + batch_size])
            total_m += m
            total_v += v
            total_c += c
            total_e += e
    ...
    log("info", f"{name}: move={total_m} violation={total_v} combat={total_c} "
                f"enforced={total_e} offset={start + consumed}")
    return True
